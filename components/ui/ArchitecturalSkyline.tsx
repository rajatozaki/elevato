...
const sectors: ElevationSector[] = [
  {
    ...
    description: 'Offline craftsmanship hindered by legacy server overhead, manual spreadsheets, disconnected inboxes, and dropped leads.',
    metrics: [
      { label: 'Speed', value: '4.6s', detail: 'Bloated CMS' },
      { label: 'Response', value: '4–12 hrs', detail: 'Manual triage' },
      { label: 'Infra', value: 'Single VPS', detail: 'No edge' },
    ],
    techStack: ['PHP / MySQL', 'Manual CSVs', 'Email Inboxes'],
  ...
  },
  {
    ...
    description: 'Next.js App Router engineering with global Vercel edge deployment, sub-second TTFB, 100% Core Web Vitals, and mobile-first UX.',
    metrics: [
      { label: 'Speed', value: '0.4s', detail: 'Instant hydration' },
      { label: 'Score', value: '100/100', detail: 'Core Web Vitals' },
      { label: 'Edge', value: '300+ Cities', detail: 'Global CDN' },
    ],
    techStack: ['Next.js 15', 'TypeScript', 'Tailwind', 'Vercel'],
    infrastructure: ['Modular Glass HQ', 'Photovoltaic Solar', 'Pulsing Fiber Trunk', 'Weather Radar'],
  },
  {
    id: 'scale',
    stageNum: '03',
    ...
    description: 'High-throughput serverless webhook pipelines qualifying incoming prospects, updating CRM databases, and dispatching instant alerts.',
    metrics: [
      { label: 'Alerts', value: '< 45 Sec', detail: 'Instant push' },
      { label: 'Sync', value: 'Realtime', detail: 'Zero copy' },
      { label: 'Saved', value: '14+ hrs/wk', detail: 'Automated admin' },
    ],
    techStack: ['Supabase PG', 'Serverless Hooks', 'Telegram API', 'n8n'],
    infrastructure: ['Twin Diagrid Towers', 'Suspension Skybridge', 'Express Glass Lifts', 'Holo HUD'],
  },
  {
    id: 'pinnacle',
    stageNum: '04',
    ...
    description: 'A sovereign digital acquisition fortress. 100% repository code ownership transferred to your GitHub, direct founder access, zero lock-in.',
    metrics: [
      { label: 'Code IP', value: '100% GitHub', detail: 'Full asset transfer' },
      { label: 'Lock-in', value: '0.0%', detail: 'Zero dependency' },
      { label: 'Direct SLA', value: 'Yash & Rajat', detail: 'Founder builders' },
    ],
    techStack: ['Proprietary IP', 'CI/CD Pipelines', 'Cloudflare', 'Founder SLA'],
...
export function ArchitecturalSkyline({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("w-full select-none relative flex flex-col gap-2.5", className)}>
      {/* Compact Top Cyber Telemetry & Control Ribbon */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 rounded-xl bg-surface/90 border border-border-subtle backdrop-blur-md shadow-2xs">
          ...
          {sectors.map((sector) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "px-2.5 py-1 rounded-lg text-xs font-mono transition-all duration-150 flex items-center gap-1.5 relative",
                  isSelected
        ...
        {/* Cyber Layer Filters & Controls */}
        <div className="flex items-center gap-1.5 text-xs font-mono">
          <button
            onClick={() => setShowFlightLanes(!showFlightLanes)}
            className={cn(
              "px-2 py-0.5 rounded-md border text-[10px] transition-colors hidden sm:flex items-center gap-1",
              showFlightLanes
          ...
          <button
            ...
            className={cn(
              "px-2 py-0.5 rounded-md border text-[10px] transition-colors flex items-center gap-1 font-bold",
              isAnimationActive
          ...
          >
            <span className={cn("w-1 h-1 rounded-full", isAnimationActive ? "bg-dark animate-ping" : "bg-text-tertiary")} />
            <span>{isAnimationActive ? "LIVE" : "PAUSED"}</span>
          </button>

          {/* Current Altitude Datum Pill */}
          <div className="flex items-center gap-1 bg-surface-elevated px-2 py-0.5 rounded-md border border-border-subtle text-[11px]">
      ...
      {/* Sleek Compact Cybercity Architectural Vector Canvas */}
      <div className="relative w-full rounded-2xl border border-border-subtle bg-surface/75 p-2 sm:p-3 overflow-hidden shadow-inner group">
        ...
        >
          ...
          <g className="opacity-20 dark:opacity-25" stroke="currentColor" strokeWidth="0.5">
            ...
            <text x="25" y="118" fontSize="6.5" fontFamily="monospace" fill="currentColor" opacity="0.6">EL +180m</text>
            <text x="25" y="73" fontSize="6.5" fontFamily="monospace" fill="currentColor" opacity="0.6">EL +540m</text>
            <text x="25" y="28" fontSize="6.5" fontFamily="monospace" fill="#1DB954" fontWeight="bold">EL +1180m</text>
          </g>
          ...
          {showFlightLanes && (
            <g>
              ...
              <g className="anim-drone-e">
                ...
                <line x1="-9" y1="-2" x2="9" y2="-2" stroke="#1DB954" strokeWidth="0.8" />
          ...
          <g stroke="currentColor" strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round" className="opacity-95">
            ...
            >
              ...
              {/* Building 1B: 2-Story Concrete Outpost & HVAC Platform */}
              ...
              {/* Active Sector Bracket */}
              {activeSectorId === 'foundation' && (
                <rect x="30" y="92" width="290" height="72" stroke="#1DB954" strokeWidth="1" strokeDasharray="4 4" fill="#1DB954" fillOpacity="0.03" rx="4" />
              )}
            </g>
            ...
            >
              ...
              {/* Ribbon Windows with Server Blade Glow */}
              ...
              {activeSectorId === 'modernization' && (
                <rect x="330" y="38" width="325" height="126" stroke="#1DB954" strokeWidth="1" strokeDasharray="4 4" fill="#1DB954" fillOpacity="0.03" rx="4" />
              )}
            </g>
            ...
            >
              ...
              {activeSectorId === 'scale' && (
                <rect x="680" y="0" width="340" height="164" stroke="#1DB954" strokeWidth="1" strokeDasharray="4 4" fill="#1DB954" fillOpacity="0.03" rx="4" />
              )}
            </g>
            ...
            >
              ...
              {activeSectorId === 'pinnacle' && (
                <rect x="1015" y="0" width="415" height="164" stroke="#1DB954" strokeWidth="1" strokeDasharray="4 4" fill="#1DB954" fillOpacity="0.03" rx="4" />
              )}
            </g>
          </g>
          ...
          {/* =========================================================================
             ...
             ========================================================================= */}
          <g fontSize="7.5" fontFamily="monospace" fill="currentColor" opacity="0.7" stroke="none">
            <text x="50" y="178">01 // GROUND ZERO [EL +0.0m]</text>
            <text x="420" y="178">02 // CLOUD EDGE FACILITY [EL +180.0m]</text>
            <text x="750" y="178">03 // COMMERCIAL ARCOLOGY [EL +540.0m]</text>
            <text x="1110" y="178">04 // ZENITH APEX CITADEL [EL +1180.0m]</text>
          </g>
          <g fontSize="6.5" fontFamily="monospace" fill="#1DB954" fontWeight="bold" opacity="0.9" stroke="none">
            <text x="50" y="189">STATUS: LEGACY / MANUAL TRIAGE</text>
            <text x="420" y="189">STATUS: NEXT.JS SSR 0.4s SPEED</text>
            <text x="750" y="189">STATUS: REALTIME AI EVENT PIPELINE</text>
            <text x="1110" y="189">STATUS: 100% PROPRIETARY ASSET</text>
          </g>
        </svg>
      </div>

      {/* Sleek Compact Diagnostic Telemetry Console */}
      <div className="p-3.5 sm:p-4 rounded-xl border border-accent/25 bg-surface/90 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="max-w-2xl">
          {/* Header Tag */}
          <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-accent-dark dark:text-accent font-semibold mb-1">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 shrink-0">
          {activeSector.metrics.map((m) => (
            <div
              key={m.label}
              className="px-3 py-1.5 rounded-lg bg-surface-elevated/80 border border-border-subtle flex flex-col justify-center min-w-[90px]"
            >
              <span className="text-[9px] font-mono text-text-tertiary uppercase tracking-wider">
                {m.label}
              </span>
              <span className="text-sm sm:text-base font-bold font-mono text-text-primary text-accent-dark dark:text-accent">
                {m.value}
              </span>
            </div>
          ))}
